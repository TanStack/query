...
React Query provides three different network modes to distinguish how [Queries](https://tanstack.com/query/v4/docs/guides/queries) and [Mutations](https://tanstack.com/query/v4/docs/guides/mutations) should behave if you have no network connection. This mode can be set for each Query / Mutation individually, or globally via the query / mutation defaults.
...
In this mode, Queries and Mutations will not fire unless you have network connection. This is the default mode. If a fetch is initiated for a query, it will always stay in the `state` (`loading`, `error`, `success`) it is in if the fetch cannot be made because there is no network connection. However, a [fetchStatus](https://tanstack.com/query/v4/docs/guides/queries#fetchstatus) is exposed additionally. This can be either:
...
If a query runs because you are online, but you go offline while the fetch is still happening, React Query will also pause the retry mechanism. Paused queries will then continue to run once you re-gain network connection. This is independent of `refetchOnReconnect` (which also defaults to `true` in this mode), because it is not a `refetch`, but rather a `continue`. If the query has been [cancelled](https://tanstack.com/query/v4/docs/guides/query-cancellation) in the meantime, it will not continue.
...
The [React Query Devtools](https://tanstack.com/query/v4/docs/devtools) will show Queries in a `paused` state if they would be fetching, but there is no network connection. There is also a toggle button to _Mock offline behavior_. Please note that this button will _not_ actually mess with your network connection (you can do that in the browser devtools), but it will set the [OnlineManager](https://tanstack.com/query/v4/docs/reference/onlineManager) in an offline state.
