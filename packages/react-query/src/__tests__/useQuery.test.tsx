...
describe('useQuery', () => {
  ...
  it('should refetch fresh query on focus when `refetchOnWindowFocus` is set to `always`', async () => {
    ...
    await vi.waitFor(() =>
      expect(
        rendered.getByText('data: 0, isFetching: false'),
      ).toBeInTheDocument(),
    )
    ...
    await vi.waitFor(() =>
      expect(
        rendered.getByText('data: 1, isFetching: false'),
      ).toBeInTheDocument(),
    )
  })
  ...
  it('should continue retries when observers unmount and remount while waiting for a retry (#3031)', async () => {
    ...
    await vi.waitFor(() =>
      expect(
        rendered.getByRole('button', { name: /show/i }),
      ).toBeInTheDocument(),
    )
  ...
  it('should show the correct data when switching keys with initialData, placeholderData & staleTime', async () => {
    ...
    await vi.waitFor(() =>
      expect(
        rendered.getByText('Current Todos, filter: all'),
      ).toBeInTheDocument(),
    )
