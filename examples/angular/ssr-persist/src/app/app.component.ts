  ...
  template: `
    <main class="page">
      <header class="hero">
        ...
        <p class="lede">
          Posts use <code>queryKey: ['posts']</code> and run on the server.
          Configure the persister with
          <code>dehydrateOptions.shouldDehydrateQuery</code> if you want only
          <code>client-persist</code> keys in <code>localStorage</code>.
        </p>
        <p class="lede hint">
          The panel below is mounted only in the browser via
          <code>afterNextRender</code>, so its query never runs during SSR.
      ...
      <section class="region">
        ...
        @if (showClientDemo()) {
          ...
          <p class="placeholder">
            ...
            <code>client-persist-demo</code> component yet — it is created after
            the first browser frame via <code>afterNextRender</code>.
          </p>
        }
      </section>
    </main>
