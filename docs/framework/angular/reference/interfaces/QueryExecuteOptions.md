...
| <a id="staletime"></a> `staleTime?` | [`StaleTimeFunction`](../type-aliases/StaleTimeFunction.md)\<`TQueryFnData`, `TError`, `TQueryData`, `TQueryKey`\> | The time in milliseconds after data is considered stale. If the data is fresh it will be returned from the cache. |
