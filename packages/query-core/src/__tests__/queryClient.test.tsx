...
describe('queryClient', () => {
  ...
  describe('cache key config', () => {
    ...
    it('should use separate query and mutation cache serializers', () => {
      ...
      expect(testClient.getQueryCache().getAll()[0]?.queryKey).toEqual([
        'date',
        date.toISOString(),
      ])
      expect(
        testClient.getMutationCache().getAll()[0]?.options.mutationKey,
      ).toEqual(['date', date.getTime()])
    })
    ...
    it('should expose the serialized query key everywhere', async () => {
      ...
      const queryFn = vi.fn((_context: any) => 'data')

      expect(
        testClient.defaultQueryOptions({ queryKey: key }).queryKey,
      ).toEqual(serializedKey)

      await testClient.query({ queryKey: key, queryFn })

      expect(queryFn.mock.calls[0]?.[0].queryKey).toEqual(serializedKey)
      expect(testClient.getQueryCache().getAll()[0]?.queryKey).toEqual(
        serializedKey,
      )
    })
    ...
    it('should support re-serializing canonical keys in setQueriesData', () => {
      const valueSerializer = vi.fn((value: unknown) =>
    ...
    it('should accept defaulted QueryCache.build options', () => {
      ...
      expect(query.queryKey).toEqual(['maps', [['a', 1]]])
      expect(hashFn).toHaveBeenCalledTimes(1)
    })
  ...
  describe('setQueryDefaults', () => {
    ...
    it('should be able to override defaults', async () => {
      const key = queryKey()
      queryClient.setQueryDefaults(key, { queryFn: () => 'data' })
    ...
    it('should match the query key partially', async () => {
      const key = queryKey()
      queryClient.setQueryDefaults([key], { queryFn: () => 'data' })
    ...
    it('should also set defaults for observers', () => {
      const key = queryKey()
  ...
  describe('defaultQueryOptions', () => {
    it('should default networkMode when persister is present', () => {
      expect(
        new QueryClient({
          defaultOptions: {
            queries: {
    ...
    it('should not default networkMode without persister', () => {
      expect(
        new QueryClient({
          defaultOptions: {
            queries: {
    ...
    it('should not default networkMode when already present', () => {
      expect(
        new QueryClient({
          defaultOptions: {
            queries: {
  ...
  describe('isMutating', () => {
    it('should return length of mutating', async () => {
  ...
  describe('ensureQueryData', () => {
    ...
    it('should return the cached query data if the query is found and preFetchQuery in the background when revalidateIfStale is set', async () => {
      const TIMEOUT = 10
      const key = queryKey()
  ...
  describe('refetchQueries', () => {
    ...
    it('should be able to refetch all active and inactive queries (queryClient.refetchQueries({ type: "all" }))', async () => {
      const key1 = queryKey()
      const key2 = queryKey()
      const queryFn1 = vi
        .fn<(...args: Array<unknown>) => string>()
  ...
  describe('invalidateQueries', () => {
    ...
    it('should not refetch disabled inactive queries even if "refetchType" is "all', async () => {
      const queryFn = vi
        .fn<(...args: Array<unknown>) => string>()
        .mockReturnValue('data1')
      const observer = new QueryObserver(queryClient, {
    ...
    it('should not refetch inactive queries that have a skipToken queryFn even if "refetchType" is "all', async () => {
      const key = queryKey()
      const observer = new QueryObserver(queryClient, {
        queryKey: key,
        queryFn: skipToken,
  ...
  describe('focusManager and onlineManager', () => {
    ...
    it('should resume paused mutations in parallel', async () => {
      onlineManager.setOnline(false)
    ...
    it('should notify queryCache after resumePausedMutations has finished when coming online', async () => {
      const key = queryKey()
    ...
    it('should not notify queryCache and mutationCache after multiple mounts/unmounts', () => {
      const testClient = new QueryClient()
      testClient.mount()
      testClient.mount()
      testClient.unmount()
