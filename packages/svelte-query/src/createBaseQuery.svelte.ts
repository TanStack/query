...
>(
  ...
  // ...and because the above only returns a cleanup from its second run onwards, tear the initial
  // subscription down with an effect of its own, so disposing an `$effect.root` unsubscribes it.
  try {
