...
describe(
  ...
  () => {
    ...
    it('should be able to remove a query', async () => {
      ...
      function Page() {
        ...
        return (
          <div>
            <button
              onClick={() => queryClient.removeQueries({ queryKey: key })}
            >
              remove
            </button>
            <button onClick={() => rerender({})}>rerender</button>
    ...
    it('select should always return the correct state', async () => {
      ...
      function Page() {
        const [count, inc] = React.useReducer((prev) => prev + 1, 2)
        const [forceValue, forceUpdate] = React.useReducer(
          (prev) => prev + 1,
          1,
        )
        ...
        function Page() {
          ...
          return (
            <div>
              <button onClick={() => refetch()}>refetch</button>
              <button
                onClick={() => queryClient.resetQueries({ queryKey: key })}
              >
                reset
              </button>
              <div>data: {state.data ?? 'null'}</div>
    ...
    describe('networkMode online', () => {
      ...
      it('online queries should pause retries if you are offline', async () => {
        ...
        await waitFor(() =>
          rendered.getByText(
            'status: error, fetchStatus: idle, failureCount: 3',
          ),
        )
