...
export const Toc = () => {
  const headings = useTocHighlight(styles.contents__link, styles['contents__link--active'], TOP_OFFSET, getHeaderAnchors, getHeaderDataFromAnchors, el => el?.parentElement?.id);
  return <ul className="space-y-3">
      {headings && headings.length > 0 && headings.map((h, i) => h.url ? <li key={`heading-${h.url}-${i}`} className={cx('text-sm ', {
      ['pl-2']: h?.depth === 3,
