<script lang="ts">
  ...
  let deepDerived = $derived({
    derived_deep_key: createQueryKeyDeepArr.test[0],
  })
...
<div>
  ...
  <h4>
    ...
    <ul>
      ...
      <li>Object Key{JSON.stringify(createQueryKeyDeepArr)}</li>
    </ul>
  </h4>
  ...
  <pre>Result: {JSON.stringify(data.data, null, 3)}</pre>
  <hr />
