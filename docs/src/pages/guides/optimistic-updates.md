...
useMutation(updateTodo, {
  ...
  onMutate: async newTodo => {
    ...
    const previousTodos = getQueryData(environment, 'todos')

    // Optimistically update to the new value
    setQueryData(environment, 'todos', old => [...old, newTodo])
  ...
  onError: (err, newTodo, context) => {
    setQueryData(environment, 'todos', context.previousTodos)
  },
...
useMutation(updateTodo, {
  ...
  onMutate: async newTodo => {
    // Cancel any outgoing refetches (so they don't overwrite our optimistic update)
    await cancelQueries(environment, ['todos', newTodo.id])

    // Snapshot the previous value
    const previousTodo = getQueryData(environment, ['todos', newTodo.id])

    // Optimistically update to the new value
    setQueryData(environment, ['todos', newTodo.id], newTodo)
  ...
  // Always refetch after error or success:
  onSettled: newTodo => {
    invalidateQueries(environment, ['todos', newTodo.id])
  },
})
```
