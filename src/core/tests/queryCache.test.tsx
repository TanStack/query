...
describe('queryCache', () => {
  ...
  describe('findAll', () => {
    test('should filter correctly', async () => {
      ...
      expect(queryCache.findAll([key1], { exact: false })).toEqual([query1])
      ...
      expect(queryCache.findAll([{ b: 'b' }], { exact: false })).toEqual([query3])
      ...
      expect(queryCache.findAll([{ a: 'a' }], { stale: false, exact: false })).toEqual([
        query3,
      ])
      expect(queryCache.findAll([{ a: 'a' }], { stale: true })).toEqual([])
      expect(queryCache.findAll([{ a: 'a' }], { active: true })).toEqual([])
      expect(queryCache.findAll([{ a: 'a' }], { inactive: true, exact: false })).toEqual([
        query3,
      ])
      expect(
        queryCache.findAll({ predicate: query => query === query3 })
      ).toEqual([query3])
      expect(queryCache.findAll('posts', { exact: false })).toEqual([query4])
    })
  })
