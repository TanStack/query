...
export const getStaticProps = async ({
  ...
  const {
    users
  } = await getNotionUsers(post.Authors || []);
...
const RenderPost = ({
  ...
  if (!post) {
    return <div className={blogStyles.post}>
        <p>
  ...
  return <div className="h-full min-h-full">
      <Banner />
      ...
      {preview && <div className={blogStyles.previewAlertContainer}>
      ...
      <div className="container max-w-3xl  mx-auto px-4 sm:px-6 lg:px-8 max-w-screen pb-12 pt-6">
        <div className="my-10 space-y-4">
          <div className=" flex items-center">
            {post.Authors.length > 0 && <div className="authors text-gray-700 mr-1">
                By {post.Authors.join(' ')}{' '}
              </div>}
            {post.Date && <div className="posted text-gray-700">
                {' '}
            ...
            <div className={`${postStyles.post} ${styles['markdown']} `}>
              ...
              if (listTagName && (isLast || !isList)) {
                toRender.push(React.createElement(listTagName, {
                  ...
                  const createEl = item => React.createElement(components.li || 'ul', {
                    key: item.key
                  }, item.children, item.nested.length > 0 ? React.createElement(components.ul || 'ul', {
                    key: item + 'sub-list'
              ...
              switch (type) {
                ...
                case 'embed':
                  {
                    ...
                    if (!isImage && !value.file_ids) {
                      ...
                      // notion resource
                      child = <Comp key={!useWrapper ? id : undefined} src={`/api/asset?assetUrl=${encodeURIComponent(display_source)}&blockId=${id}`} controls={!isImage} alt={`An ${isImage ? 'image' : 'video'} from Notion`} loop={!isImage} muted={!isImage} autoPlay={!isImage} style={childStyle} />;
                    }

                    toRender.push(useWrapper ? <div style={{
                      paddingTop: `${Math.round(block_aspect_ratio * 100)}%`,
                      position: 'relative'
                    }} className="asset-wrapper" key={id}>
                          {child}
                        </div> : child);
                    break;
                  }
                ...
                case 'code':
                  {
                    if (properties.title) {
                      ...
                      if (language === 'LiveScript') {
                        ...
                        toRender.push(<ReactJSXParser key={id} jsx={content} components={components} componentsOnly={false} renderInpost={false} allowUnknownElements={true} blacklistedTags={['script', 'style']} />);
                      } else {
                        toRender.push(<components.Code key={id} language={language || ''}>
                            {content}
                          </components.Code>);
                      }
                    }
                ...
                case 'callout':
                  {
                    toRender.push(<div className="callout" key={id}>
                ...
                case 'tweet':
                  {
                    if (properties.html) {
                      toRender.push(<div dangerouslySetInnerHTML={{
