...
Waiting for queries to become stale before they are fetched again doesn't always work, especially when you know for a fact that a query's data is out of date because of something the user has done. For that purpose, an `invalidateQueries` function is provided that lets you intelligently mark queries as stale and potentially refetch them too!

```js
// Invalidate every query in the cache
invalidateQueries(environment)
...
// Get environment from the context
...
```js
invalidateQueries(environment, ['todos', { type: 'done' }])
...
```js
invalidateQueries(environment, 'todos', { exact: true })
